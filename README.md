...
🔗 **Guest mode (seeded demo data):** https://creative-atlas.web.app/?guest=1
...
2.  **Configure Environment Variables:**
    ...
    *   Open `server/.env` and fill in the following values:
        ...
        *   The Firebase Hosting rewrite for `/share/**` now targets the Cloud Function `shareMetadata` (`functions/index.js`), which fetches the shared project payload from the App Engine API and renders the metadata HTML. The function defaults to `https://creative-atlas.uc.r.appspot.com` but you can override the upstream host by setting the `APP_ENGINE_API_BASE_URL` environment variable before deploying the function.
