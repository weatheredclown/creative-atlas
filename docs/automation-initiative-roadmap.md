...
- Investigate Firebase Hosting deploy instability after migrating share rewrites off Cloud Run.
  ...
  - Share metadata function deploy now also uses `--except extensions` across merge and preview workflows so the service account skips Firebase Extensions enumeration and retains least-privilege access. Monitor upcoming runs to confirm the new flag eliminates the 403 regression before expanding permissions.
