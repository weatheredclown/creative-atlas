...
- Add regression coverage for artifact normalization so partial records from the data API never crash the workspace editors.
  - Add tests around the new residue cache that preserves mismatched artifact tags/relations so we confirm editors warn instead of dropping data when normalization kicks in.
