...
- Automate publish pipeline: convert the “Publish Site” action into a backend export that builds static bundles, deploys to durable storage, and exposes job status (GitHub error reporting & build orchestration hardened). (Resolved TypeScript typing issues in the GitHub publish tests/queue so the flow builds again.)
