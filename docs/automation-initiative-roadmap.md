...
- Investigate Firebase Hosting deploy instability after migrating share rewrites off Cloud Run.
  ...
  - Production Hosting workflow no longer relies on `FirebaseExtended/action-hosting-deploy`; the job now shells out to `firebase deploy --only hosting` while relying on Cloud Functions deployments handled separately via gcloud. Confirm upcoming merge runs no longer see Firebase Extensions permission failures before duplicating the pattern across preview channels.
  - Share metadata function deploy now runs through `gcloud functions deploy` in merge and preview workflows so we bypass Firebase CLI's Firebase Extensions discovery. Monitor upcoming runs to ensure the direct gcloud path keeps the function updated without reintroducing permissions drift.
