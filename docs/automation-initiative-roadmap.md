...
- Automate publish pipeline: convert the “Publish Site” action into a backend export that builds static bundles, deploys to durable storage, and exposes job status (GitHub error reporting & build orchestration hardened). (Queue utilities now expose both a fire-and-forget worker and a UI-orchestrated variant; next slice is surfacing status endpoints.)
